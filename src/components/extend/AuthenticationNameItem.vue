<template>
	<div class="pu">
            ...
            <div class="container">
                ...
                <button @click="showPlugin" class="btn tijiao">立即验证</button>
            </div>
	...
	export default {
        ...
        data(){
            return{
                ...
                address:'',province:'',city:'',bankNo:'',bankName:'',type:'',logo:'',status:''
            }
        },
        components: {
            XButton,
        ...
        created(){
            ...
            this.bankName = this.$route.query.adress.bank
            this.type = this.$route.query.adress.type
            this.logo = this.$route.query.adress.logo
            this.status = this.$route.query.status

            // console.log(this.$route.query.adress.bankcard)
            this.bankNo = this.$route.query.adress.bankcard
        },
        methods: {
            history() {
                history.go(-1)
            ...
            showPlugin() {
                ...
                this.$http.post(httpUrl.dev + '/puman/api/system/bank_check',{ 'token':this.token1, 'bankNo':this.bankNo, 'realName':name }, {emulateJSON: true}) 
                .then(response => {
                    ...
                    if(response.body.code == 10000){
                        ...
                        if (response.body.result.error_code == 0) {
                            ...
                            var bankcard1 = this.bankNo
                            var type1 = encodeURI(encodeURI(this.type))
                            ...
                            set('type', type1, 1)
                            this.$storage.setStore('logo',this.logo)
                            var bankCity = this.city || shi
                            var accountBankName = kaihu
                            var bankProvince = this.province || shen
